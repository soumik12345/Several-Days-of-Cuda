...
@click.option(
    '--memory_allocation', '-m',
    type=click.Choice(
        ['auto', 'manual'], case_sensitive=False
    ), help='Enable Automatic Memory Allocation')
def main(kernel_name, source_name, memory_allocation):
    template_path = './src/lib/BasicExamples/'
    template_source = open(template_path + 'ArrayManipulation.cuh').read() if memory_allocation == 'auto' \
        else open(template_path + 'ArrayManipulationManualMemoryAllocation.cuh').read()
